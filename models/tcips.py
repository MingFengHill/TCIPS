...
class MTLHead(ptf_model.PointModule):
    def __init__(self,
        ...
        self.embedding_module = nn.Sequential(
            nn.Conv1d(feat_channels*2, feat_channels*2, 1),
        ...
        self.primitives_module = nn.Sequential(
            ...
            nn.ReLU(True),
            nn.Dropout(drop_out),
            nn.Conv1d(feat_channels, num_primitives, 1),
            nn.LogSoftmax(dim=1)
        )
        # purity prediction
    ...
    def forward(self, feat):
        ...
        shape = final_feats["semantic"].feat.shape
        B, N, C = self.batch_size, int(shape[0]/self.batch_size), shape[1] 
        # embedding
        embedding_pre = torch.cat((final_feats["offset"].feat, 
                                   final_feats["instance"].feat),
                                   dim=1)
        embedding_pre = embedding_pre.reshape(B, N, C*2)
        embedding_pre = embedding_pre.transpose(1, 2)
        embedding_pre = self.embedding_module(embedding_pre)
        # offset prediction
        offset_pre = final_feats["offset"].feat.reshape(B, N, C)
        offset_pre = offset_pre.transpose(1, 2)
        offset_pre = self.offset_module(offset_pre)
        # primitives prediction
        primitives_pre = final_feats["semantic"].feat.reshape(B, N, C)
        primitives_pre = primitives_pre.transpose(1, 2)
        primitives_pre = self.primitives_module(primitives_pre)
        # purity prediction
        purity_pre = final_feats["purity"].feat.reshape(B, N, C)
        purity_pre = purity_pre.transpose(1, 2)
...
class MTLNetBase(ptf_model.PointModule):
    ...
    def forward(self, feat):
        data_dict = self.sed_data_adaptation(feat)
        feat = self.backbone(data_dict)
        out = self.head(feat)
        return out

    def sed_data_adaptation(self, feat, grid_size=0.01):
        feat = feat.transpose(1,2)
        points = feat[:, :, 0:3]

        # (batch, point size, channel)
        shape = feat.shape
        feat = feat.reshape(shape[0]*shape[1], shape[2])

        offset = [i * shape[1] for i in range(1, shape[0] + 1)]
        offset = torch.tensor(offset).cuda()

        shape = points.shape
        coord = points.reshape(shape[0]*shape[1], shape[2])
