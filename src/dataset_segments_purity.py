...
class ori_simple_data(Dataset_std):
    def __init__(self, ids=[], prefix="", if_normals=False, if_train=False, aug=True, noise=False, noise_level=0, starts=0, input_num=10000, if_offset=False, if_purity=False, neighborhood_size='50', if_weight=False):
        super().__init__()
    ...
    def load_hdf5(self, ):
        with h5py.File(self.prefix + "data_parsenet/{}_data_purity_{}.h5".format("train" if self.if_train else "test", self.neighborhood_size), "r") as hf:
            ...
            if self.if_purity:
                purity = np.array(hf.get("purity"))
            if self.if_weight:
                purify_weight = np.array(hf.get("purify_weight"))
        points = points[self.rows].astype(np.float32)
        labels = labels[self.rows]
        if self.if_purity:
            self.purity = purity[self.rows].astype(np.float32)
        if self.if_weight:
            self.purify_weight = purify_weight[self.rows].astype(np.float32)
    ...
    def __getitem__(self, index):
        ...
        if self.if_purity:
            _purity = self.purity[index]
        if self.if_weight:
            _purify_weight = self.purify_weight[index]
        ...
        if not self.if_train and self.aug:
            if not self.if_normals:
                _points = self.myValAug.augment(_points.reshape(1, 10000, 3))[0]
            else:
                tmp = self.myValAug.augment([_points.reshape(1, 10000, 3), _normals.reshape(1, 10000, 3)])
                _points = tmp[0][0]
                _normals = tmp[1][0]
        ...
        if self.noise and self.noise_level != -1:
            ...
            # Add Gaussian noise in the normal direction
            clip_ = sigma*3
            noise = _normals * np.clip(
                    np.random.randn(_points.shape[0], 1) * sigma,
                    a_min=-clip_,
                    a_max=clip_)
            _points = _points + noise
        ...
        if self.if_offset:
            pt_mean = np.ones((_points.shape[0], 3), dtype=np.float32) * -10000.0
            instance_pointnum = []
            instance_cls = []
            instance_num = max(int(_labels.max()) + 1, 0)
            for i_ in range(instance_num):
                inst_idx_i = np.where(_labels == i_)
                xyz_i = _points[inst_idx_i]
                ...
                instance_cls.append(_labels[cls_idx])
            has_negative = np.any(np.isin(pt_mean, -10000.0))
            if has_negative:
                exit
            pt_offset_label = pt_mean - _points
            return_items.append(pt_offset_label.astype(np.float32))
            
        if self.if_purity:
            return_items.append(_purity.astype(np.float32))
            
        if self.if_weight:
            return_items.append(_purify_weight.astype(np.float32))
