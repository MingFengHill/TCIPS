...
from src.augment_utils import rotate_perturbation_point_cloud, jitter_point_cloud, shift_point_cloud, \
    random_scale_point_cloud, rotate_point_cloud, MyAugment, PointTransformerAugment
...
class my_simple_data(Dataset_std):
    def __init__(self, ids=[], prefix="", if_normals=False, if_train=False, ret_edges1w=False, aug=True, noise=False, noise_level=0, if_offset=False, if_purity=False, neighborhood_size='50', if_weight=False):
        super().__init__()
    ...
    def load_hdf5(self, ):
        with h5py.File(self.prefix + "data/{}_data_withEdge_purity_{}.h5".format("train" if self.if_train else "test", self.neighborhood_size), "r") as hf:
            # print("hdf5 load ok")
